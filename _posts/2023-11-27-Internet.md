...
> Popcorn Hack
 ...
 - ASCII falls into the presentation layer (Layer 6) of the OSI model, and another example of a protocol from that layer is the JPEG image compression standard.
#### TCP/IP Model
...
> Popcorn Hack
 ...
 - On a streaming service when its quick
 - When might we use TCP?
 - When you are banking
...
<br>
- 1.1.1.1.1: No, because it has too many parts (segments).
- 23.23.23.23: Yes, this is a good IP address.
- 134.492.100.0: No, because the middle part is too big.
- 255.256.55.255: No, because the second part is too big.
- 2.93.255.19: Yes, this is a good IP address.
...
- Unicast
- Broadcast
- He is sending a broadcast because he is sending a message to everyone at one time 
...
1. ASCII doesn't operate on a specific OSI layer; it's a character encoding standard.
2. FTP works at the Application Layer (Layer 7) for file transfer.
3. TLS operates at the Transport Layer (Layer 4) to ensure secure communication.
4. USB is a hardware standard and doesn't align with a specific OSI layer.
...
Telnet works at the top layer (Application Layer) of the OSI model, letting you control another computer remotely, and this layer deals with user-friendly network services like remote access.
...
- Bob should buy the domain "best.com," and the subdomains would be "bob.is.the" and "bob."
<br>
 - What protocol does HTTPS use that HTTP doesn't?
 - HTTPS uses a security protocol called TLS for encryption, which HTTP doesn't have.
...
<br>
 ...
 - HTTPS ensures secure data transfer online, protecting against eavesdropping, but it might slightly slow down website performance due to encryption processes and can involve additional costs for certificates.
...
-  Bob should choose UDP for his real-time video streaming because it's faster and better suited for quick, continuous data like video.

-  However, with UDP, there's a risk of some data loss, so if a few pieces of the video get lost, it might cause glitches or interruptions in the stream.
...
<br>
 ...
 - TCP's error checking is important to make sure all data reaches its destination correctly. For instance, if Bob sends a file, TCP uses a system of acknowledgments and packet numbering to ensure that every part arrives safely and in the right order.
...
<br>
 ...
 - In TCP, Server B confirms receiving packets by sending acknowledgments to Server A before more data is sent. UDP lacks this confirmation, allowing Server A to send data without waiting for acknowledgment. This process ensures reliable and ordered data delivery between the servers.